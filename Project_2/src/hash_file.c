...
HT_ErrorCode CreateNewBucket( int filedesc, Record record, int bucket, TupleId* tupleid, UpdateRecordArray* updaterecordarray)
{
  ...
  //array with change
  ChangeTable* change_table = malloc( ((BF_BLOCK_SIZE -sizeof(int))/sizeof(record))*sizeof(struct changetable) );
...
{ 
  ...
  CreateNewBucket( filedesc, record, bucket_b, tupleid, updaterecordarray);
  BF_Block_Destroy( &block);
  return HT_OK;
}
